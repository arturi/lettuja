...
function resizeAndMove (folder, imgSize, file) {
  ...
  if (file.mimetype === 'image/jpeg' ||
    ...
    const newName = originalName.replace(/\W+/g, '-').toLowerCase() + Date.now() + originalExtension.toLowerCase()
    const dstImgPath = conf.outputMediaDirectory + '/' + folder + '/' + newName
    const imgPath = '/media/' + folder + '/' + newName
  ...
  // otherwise just move
    ...
    const dstImgPath = conf.outputMediaDirectory + '/' + folder + '/' + originalName + originalExtension
    const imgPath = '/media/' + folder + '/' + originalName + originalExtension
